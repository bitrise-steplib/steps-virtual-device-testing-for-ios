...
| `test_devices` | Format: One device configuration per line and the parameters are separated with `,` in the order of: `deviceID,version,language,orientation` For example: `iphone8,16.6,en,portrait` `iphone8,16.6,en,landscape` Available devices and their versions (generated on 2026-07-24): ``` ┌─────────────┬────────────────────────┬────────────────┬─────────┐ │   MODEL_ID  │          NAME          │ OS_VERSION_IDS │   TAGS  │ ├─────────────┼────────────────────────┼────────────────┼─────────┤ │ ipad10      │ iPad (10th generation) │ 16.6           │         │ │ iphone11pro │ iPhone 11 Pro          │ 16.6           │         │ │ iphone14pro │ iPhone 14 Pro          │ 16.6           │ default │ │ iphone16pro │ iPhone 16 Pro          │ 18.3           │         │ │ iphone8     │ iPhone 8               │ 16.6           │         │ │ iphonese3   │ iPhone SE 3            │ 18.4,26.3      │         │ └─────────────┴────────────────────────┴────────────────┴─────────┘ ```  Device and OS Capacity (generated on 2026-07-24): ``` ┌─────────────┬────────────────────────┬───────────────┬─────────────────┐ │   MODEL_ID  │       MODEL_NAME       │ OS_VERSION_ID │ DEVICE_CAPACITY │ ├─────────────┼────────────────────────┼───────────────┼─────────────────┤ │ ipad10      │ iPad (10th generation) │ 16.6          │ Medium          │ │ iphone11pro │ iPhone 11 Pro          │ 16.6          │ Medium          │ │ iphone14pro │ iPhone 14 Pro          │ 16.6          │ Medium          │ │ iphone16pro │ iPhone 16 Pro          │ 18.3          │ Medium          │ │ iphone8     │ iPhone 8               │ 16.6          │ Medium          │ │ iphonese3   │ iPhone SE 3            │ 18.4          │ Medium          │ │ iphonese3   │ iPhone SE 3            │ 26.3          │ Medium          │ └─────────────┴────────────────────────┴───────────────┴─────────────────┘ ```  Google regularly adds and retires Firebase Test Lab devices, so the tables above can go out of date. There are three ways to check the current catalog:  - **Firebase console** (no CLI needed): open [Test Lab](https://console.firebase.google.com/project/_/testlab) and start a test run — the available devices are shown in the **Select dimensions** step. - **Google APIs Explorer** (no CLI and no Firebase project needed): call [`testEnvironmentCatalog.get` with `environmentType=IOS`](https://developers.google.com/apis-explorer/#p/testing/v1/testing.testEnvironmentCatalog.get?environmentType=IOS). - **gcloud CLI** locally, version 417.0.0 or newer, signed in with `gcloud auth login` and a Google Cloud project set:   ```   gcloud firebase test ios models list   gcloud firebase test ios list-device-capacities   ```  | required | `iphone16pro,18.3,en,portrait` |
